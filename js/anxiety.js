...
let anxietyquiz = document.querySelector(".anxietyQuiz");
...
// background.style.backgroundImage=colors[colorIndex];
//
...
start.addEventListener("click", () => {
    anxietyquiz.style.display = "flex";
...
let loadData = () => {
    if (index < MAX_question) {
        ...
        $(".progressText").text(Number(index + 1) + " / 7");
...
$(".choice").click(function (event) {
    setTimeout(function () {
        ...
        ProgessValue.style.width = `${(index/MAX_question)*118}%`

    }, 400)
    //    progressText.innerText = `${index+1} of ${MAX_question}`;
...
function goToResultpage() {
    setTimeout(function () {
    index = 0;
    location.href = "./result_anxiety.html"
         }, 50)
}
