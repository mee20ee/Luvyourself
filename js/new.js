...
let loadData = () => {
    if (index < MAX_question) {
        ...
        $(".progressText").text(Number(index + 1) + " /20");
    }
...
$(".choice").click(function (event) {
    setTimeout(function () {
        ...
        ProgessValue.style.width = `${(index/MAX_question)*107}%`
    }, 400)
});

//Set an initial total scores to zero
if (index < MAX_question) {
    ...
    $("#option5").click(function () {
        total_scores += scores[index].option5;
        console.log(scores[index].option5);
        console.log(total_scores);
    });
}
